...
// Token cache — survives across requests on warm Vercel instances
// Avoids hitting Microsoft on every API call (saves ~500-800ms per request)
let _cachedToken = null;
let _cachedScope = '';
let _cachedAt = 0;
const TOKEN_TTL = 45 * 60 * 1000;  // refresh every 45 min (tokens usually last 60-90)

/**
 * Exchange a refresh token for fresh Graph access + refresh tokens.
 * Caches the result so subsequent calls within TTL skip the HTTP roundtrip.
...
export async function getAccessToken(tokenData, extraScope = '') {
  ...
  const baseScope = 'openid profile email offline_access Files.Read.All Files.ReadWrite.All Sites.Read.All Sites.ReadWrite.All Mail.Read Calendars.Read User.Read';
  const scope = extraScope ? `${baseScope} ${extraScope}` : baseScope;

  // Return cached token if still fresh
  if (_cachedToken && _cachedScope === scope && (Date.now() - _cachedAt) < TOKEN_TTL) {
    _cachedToken.refresh_token = _cachedToken.refresh_token || tokenData.refresh_token;
    return _cachedToken;
  }
  ...
  // Build result and cache for subsequent requests
  const result = {
