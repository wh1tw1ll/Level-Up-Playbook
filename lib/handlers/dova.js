...
export async function fetchSheet(sheetKey) {
  ...
  if (sheetKey === '03') {
    ...
    if (sheetKey === '09') {
      ...
      rows.push({ Metric: 'Level Up Target (w/ OPOI Savings)', Value: '$248.8M' });
      ...
      rows.push({ Metric: 'P&W SD Fee (Fixed)', Value: '$1.4M' });
      rows.push({ Metric: 'Contingency (McCarthy Est.)', Value: '$26.3M' });
      rows.push({ Metric: 'OPOI Target', Value: '$68M' });
      rows.push({ Metric: 'Level Up OR Fee', Value: '$1.856M' });
      rows.push({ Metric: 'Level Up OPOI Manpower', Value: '$2.2M' });
      rows.push({ Metric: 'Level Up 1.5% OPOI Fee', Value: '$1.248M' });
      rows.push({ Metric: 'Approved Change Orders', Value: '$0' });
      rows.push({ Metric: 'Delivery Method', Value: 'Under Eval - CM Agent/GMP/DBIA' });
      rows.push({ Metric: 'Design Structure', Value: 'Owner-Direct (P&W LNTP)' });
      rows.push({ Metric: 'SD Launch Date', Value: 'Jun 29, 2026' });
