...
export async function fetchSheet(sheetKey) {
  ...
  if (sheetKey === '03') {
    ...
    const kozItems = [
      ...
      // -- Added from Greg's KozPure Draft Agenda (Jun 26) --
      { 'Action ID':'A-039', 'Action Description':'Expedite P&W AIA contract - provide draft for review; Level Up review business terms; KozPure legal review concurrently', 'Owner':'P&W', 'Due Date':'2026-06-26', 'Priority':'Critical', 'Status':'Pending', 'Depends On':'A-022' },
      { 'Action ID':'A-040', 'Action Description':'Resolve P&W fee / budget basis open question - agree which budget number the 6.5% fee is based on', 'Owner':'KozPure', 'Due Date':'2026-07-06', 'Priority':'Critical', 'Status':'Pending', 'Depends On':'A-022' },
      { 'Action ID':'A-041', 'Action Description':'Award CM preconstruction services by Jul 13 to support Sep groundbreaking - critical path deadline', 'Owner':'KozPure', 'Due Date':'2026-07-13', 'Priority':'Critical', 'Status':'Pending', 'Depends On':'A-035' },
      { 'Action ID':'A-042', 'Action Description':'Decide CM path forward after McCarthy feedback - proceed, pivot to Turner/Level 10/Swinerton, or other', 'Owner':'Level Up', 'Due Date':'2026-07-07', 'Priority':'High', 'Status':'Pending', 'Depends On':'A-035' },
      { 'Action ID':'A-043', 'Action Description':'Schedule Turner Construction follow-up - Clint Williams + Drake Costa + John Gromos', 'Owner':'Greg', 'Due Date':'2026-07-03', 'Priority':'High', 'Status':'Pending', 'Depends On':'' },
      { 'Action ID':'A-044', 'Action Description':'Follow up with Swinerton (Jeff) and SB James (Heman) for CM outreach', 'Owner':'Greg', 'Due Date':'2026-07-03', 'Priority':'Medium', 'Status':'Pending', 'Depends On':'' },
      { 'Action ID':'A-045', 'Action Description':'Obtain executed P&W contract before signing engineers/consultants - Don needs contract to proceed', 'Owner':'KozPure', 'Due Date':'2026-07-06', 'Priority':'Critical', 'Status':'Pending', 'Depends On':'A-039' },
      { 'Action ID':'A-046', 'Action Description':'Complete Level Up Dashboard launch for KozPure review', 'Owner':'Level Up', 'Due Date':'2026-06-30', 'Priority':'High', 'Status':'In Progress', 'Depends On':'' },
      { 'Action ID':'A-047', 'Action Description':'Complete DOVA master schedule for City submittal - Josh requested City-ready schedule', 'Owner':'Level Up', 'Due Date':'2026-06-30', 'Priority':'High', 'Status':'In Progress', 'Depends On':'A-033' },
    ];
    ...
    if (sheetKey === '09') {
      ...
      rows.push({ Metric: 'Budget Status', Value: 'Pending Approval - Range Est.' });
      rows.push({ Metric: 'Working Budget Range', Value: '$230M - $270M' });
      rows.push({ Metric: 'McCarthy Estimate (May 2026)', Value: '$272.9M' });
      rows.push({ Metric: 'Target Savings (OPOI + Commercial Reset)', Value: '$24.1M' });
      rows.push({ Metric: 'Committed (LOI + Precon)', Value: '$1.5M' });
      rows.push({ Metric: 'Billed to Date', Value: '$1.3M' });
      rows.push({ Metric: 'Pay App #5 Due (June)', Value: '$315.6K' });
      rows.push({ Metric: 'McCarthy SD Precon Fee', Value: '$75K' });
      rows.push({ Metric: 'Contingency (McCarthy Est.)', Value: '$26.3M' });
      rows.push({ Metric: 'Approved Change Orders', Value: '$0' });
      rows.push({ Metric: 'Delivery Method', Value: 'Under Eval - CM Agent/GMP' });
      rows.push({ Metric: 'Design Structure', Value: 'Owner-Direct' });
      rows.push({ Metric: 'Design Phase', Value: 'Schematic Design (Jun 29 - Aug 11)' });
      rows.push({ Metric: 'Days to Groundbreaking', Value: '89 days (Sep 23)' });
      rows.push({ Metric: 'Groundbreaking Target', Value: 'Sep 23, 2026' });
      rows.push({ Metric: 'Opening Target', Value: 'Mar 21, 2028' });
    } else if (sheetKey === '10') {
      rows.length = 0;
      ...
      rows.push({ Category:'Contingency', Planned:'26.3M (Est.)', Committed:'—', 'Spent to Date':'—', 'Forecast at Completion':'26.3M', Variance:'—', 'Percent Spent':'—', 'Contingency Used':'—', Status:'Estimate Only' });
          ...
          rows.length = 0;
          rows.push({ Name:'Whitney Williams', Role:'Principal-in-Charge / Ops Lead', Email:'wwilliams@levelup-pd.com' });
          rows.push({ Name:'Greg Wieting', Role:'Principal / Exec Lead / Primary KozPure Contact', Email:'gwieting@levelup-pd.com' });
          rows.push({ Name:'Justin Williams', Role:'Project Manager / Owner Scopes', Email:'' });
          rows.push({ Name:'Jordan Ward', Role:'Field Lead / Superintendent Support', Email:'jordan@levelup-pd.com' });
          rows.push({ Name:'Joshua Wood', Role:'CEO - KozPure Development', Email:'Josh@kozpure.com' });
          rows.push({ Name:'Charlie Tiwana', Role:'COO - KozPure Development', Email:'Charlie@kozpure.com' });
          rows.push({ Name:'Onkar', Role:'Finance - KozPure', Email:'Onkar@kozpure.com' });
          rows.push({ Name:'Adam Osantowski', Role:'Precon Director - McCarthy', Email:'AOsantowski@mccarthy.com' });
          rows.push({ Name:'Talbiani', Role:'Exec - McCarthy', Email:'talbiani@mccarthy.com' });
          rows.push({ Name:'Sam Kalscheur', Role:'Architect Lead - Perkins & Will', Email:'' });
          rows.push({ Name:'Scott Hooker', Role:'Principal - Buehler Engineering (Structural)', Email:'shooker@buehlerengineering.com' });
          rows.push({ Name:'Clint Williams', Role:'Construction Executive - Turner Construction', Email:'cwilliams@tcco.com' });
    }
  }
