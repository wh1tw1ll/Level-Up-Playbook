...
async function handleStatusNotePost(req, res, token, sheetId, statusNote) {
  try {
    ...
    const result = await guardedWrite({
      ...
      existingRowId: rowId,
      columns: [
        { title: 'Action ID', columnId: sheetData.columns.find(c => c.title === 'Action ID').id, type: 'TEXT_NUMBER', value: 'Status Note Update' },
        { title: 'Status Note', columnId: noteCol.id, type: noteCol.type, value: String(statusNote) },
      ],
    });

    if (!result.passed) {
      // Fallback: update via smartsheet.js wrapper
