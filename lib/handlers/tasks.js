...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
      // POST to /api/tasks/create — create a new task row
    ...
    function extractTasks(data, source) {
      ...
      return rows.map(row => {
        ...
        return {
          ...
          completedDate: resolved['Completed Date'] || null,
        };
      });
    }
...
async function handlePost(req, res, token, sheetId) {
  try {
    ...
    const newCompletedDate = body.completedDate;

    if (!newStatus && newActionItem === undefined && !newOwner && !newCategory && !newProject && !newDueDate && !newFirm && !newCompletedDate) {
...
export async function handleCreate(req, res) {
  try {
    ...
    const title = (body.actionItem || '').trim();
    if (!title) return res.status(400).json({ error: 'actionItem is required' });

    // Fetch column IDs
    const sheetData = await smartsheet.getSheetWithColumns(SHEET_ID_PROJECT);
    const cols = sheetData.columns || [];
    function colId(colTitle) {
      const c = cols.find(x => x.title === colTitle);
      return c ? c.id : null;
    }

    const cells = [];

    function addCell(colTitle, value) {
      if (!value) return;
      const id = colId(colTitle);
      if (!id) return;
      cells.push({ columnId: id, value: String(value) });
    }

    addCell('Action ID', title);
    addCell('Owner', body.owner);
    addCell('Status', body.status || 'In Progress');
    addCell('Due Date', body.dueDate);
    addCell('Project', body.project);
    addCell('Category', body.category);
    addCell('Responsible Firm(s)', body.responsibleFirm);
    addCell('Discipline', body.discipline);
    addCell('SourceRef', body.sourceRef || 'Manual: ' + new Date().toISOString().slice(0,10));

    const result = await smartsheet.addRow(SHEET_ID_PROJECT, cells);

    res.json({ success: true, rowId: result.id, message: 'Task created' });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
}