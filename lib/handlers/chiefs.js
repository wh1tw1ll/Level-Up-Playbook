...
function getWorkspaceId() {
  return process.env.SMARTSHEET_WORKSPACE_ID || '4136049782876036';
}
...
async function ssApiPost(path, body) {
  const url = 'https://api.smartsheet.com/2.0' + path;
  const res = await fetch(url, {
    method: 'POST',
    headers: {
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({ message: res.statusText }));
    throw new Error(err.message || err.result?.message || 'Smartsheet POST returned ' + res.status);
  }
  return res.json();
}

async function ssApiFetch(path, method, body) {
  const url = 'https://api.smartsheet.com/2.0' + path;
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({ message: res.statusText }));
    throw new Error(err.message || 'Smartsheet ' + method + ' returned ' + res.status);
  }
  return res.json();
}
...
async function fetchSheet(sheetKey) {
  ...
  const currencyCols = ['Planned Amount','Committed Amount','Spent to Date','Planned Cumulative','Actual Cumulative','Budget Value','Amount'];
...
export default async function handler(req, res) {
  ...
  // === ADMIN / MUTATION ENDPOINTS (POST only, for LUNA orchestration) ===
  if (req.method === 'POST') {
    const { action, name, destinationId, sheetId, newName, include } = req.body || {};
    try {
      if (action === 'createWorkspace') return res.json(await ssApiPost('/workspaces', { name }));
      if (action === 'copySheet' && sheetId && destinationId) {
        const body = { destinationType: 'workspace', destinationId: parseInt(destinationId), newName: newName || name || '' };
        const qs = include ? '?include=' + include : '';
        return res.json(await ssApiPost('/sheets/' + sheetId + '/copy' + qs, body));
      }
      if (action === 'createSheet' && name && destinationId) {
        return res.json(await ssApiPost('/workspaces/' + parseInt(destinationId) + '/sheets', { name, columns: req.body.columns || [] }));
      }
      if (action === 'addColumn' && sheetId && req.body.column) {
        return res.json(await ssApiPost('/sheets/' + sheetId + '/columns', [req.body.column]));
      }
      if (action === 'insertRows' && sheetId && req.body.rows) {
        return res.json(await ssApiPost('/sheets/' + sheetId + '/rows', req.body.rows));
      }
      if (action === 'deleteRows' && sheetId && req.body.rowIds) {
        return res.json(await ssApiFetch('/sheets/' + sheetId + '/rows?ids=' + req.body.rowIds.join(','), 'DELETE'));
      }
      if (action === 'updateRows' && sheetId && req.body.rows) {
        return res.json(await ssApiFetch('/sheets/' + sheetId + '/rows', 'PUT', req.body.rows));
      }
      if (action === 'getSheet' && sheetId) {
        return res.json(await ssApiFetch('/sheets/' + sheetId, 'GET'));
      }
      if (action === 'deleteSheet' && sheetId) {
        return res.json(await ssApiFetch('/sheets/' + sheetId, 'DELETE'));
      }
      if (action === 'listSheets' && sheetId) {
        return res.json(await ssApiFetch('/workspaces/' + parseInt(sheetId) + '/sheets', 'GET'));
      }
      if (action === 'apiGet' && req.body.path) {
        return res.json(await ssApiFetch(req.body.path, 'GET'));
      }
