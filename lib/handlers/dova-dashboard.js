...
<div id="app" class="hidden">
  <aside class="nav">
    <div class="brand">
            <img src="/assets/dova-logo.jpg" alt="DOVA" style="height:32px;width:auto"/>
          </div>
    <div class="items">
      ...
      <a data-page="permits"><span class="ic">&#9881;</span> Permits</a>
    </div>
  ...
  <main>
    ...
    <div class="wrap">
      ...
      <section id="page-permits" class="page hidden"></section>
    </div>
  </main>
</div>
...
function normalize(json){
  ...
  if(json.rows && json.columns){
    // Flat objects keyed by column title (DOVA API format)
    if(json.rows.length>0 && !json.rows[0].cells && typeof json.rows[0]==="object"){
      return json.rows;
    }
    // Smartsheet cells format (legacy)
    var byId = {};
...
function renderPermits(){
  var s08=STATE["08"]||[];
  $("page-permits").innerHTML = card("Permitting Matrix", s08.length+" items",
    table(s08, [
      {keys:["ID"],label:"ID"},
      ...
      {keys:["Status"],label:"Status",kind:"status"},
      {keys:["Application Date"],label:"Applied"},
      {keys:["Target Approval"],label:"Target"},
...
function renderCurrent(){
  ...
  else if(CURRENT==="permits") renderPermits();
}
...
function showSkeleton(){
  ...
  $("page-permits").innerHTML = skeleton("rows");
}
...
function setPage(p, el){
  CURRENT=p;
  var pages=["overview","budget","schedule","actions","permits"];
