...
function renderBudget(){
  ...
  var billedVal = "$1.3M";
  for(var i=0;i<s09.length;i++){var m=get(s09[i],["Metric"]);if(m==="Total Budget")budgetVal=get(s09[i],["Value"]);if(m.indexOf("McCarthy")>=0)mccarthyEst=get(s09[i],["Value"]);if(m.indexOf("Committed")>=0)committedVal=get(s09[i],["Value"]);if(m==="Billed to Date")billedVal=get(s09[i],["Value"])}
  html+='<div class="kpi-ticker">'
