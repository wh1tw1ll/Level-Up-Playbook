...
// data from /api/dova?sheet=NN. Navy/gold theme, client password gate, 4 pages.
// Note: the password gate is client-side and is a soft gate, not real security.
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  /* Password gate */
  #gate{position:fixed;inset:0;z-index:50;background:linear-gradient(135deg,var(--navy) 0%,var(--navy-d) 100%);display:flex;align-items:center;justify-content:center}
  .gatebox{width:340px;max-width:90vw;text-align:center;color:#fff}
  .gatebox .logo{width:74px;height:74px;border-radius:16px;margin:0 auto 18px;background:var(--gold);color:var(--navy-d);font-weight:800;font-size:28px;display:flex;align-items:center;justify-content:center;letter-spacing:1px}
  .gatebox h1{font-size:19px;font-weight:700;margin-bottom:4px}
  .gatebox p{font-size:13px;color:#c6d4e3;margin-bottom:22px}
  .gatebox input{width:100%;padding:13px 14px;border-radius:10px;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.08);color:#fff;font-size:15px;outline:none}
  .gatebox input::placeholder{color:#9fb3c8}
  .gatebox input:focus{border-color:var(--gold)}
  .gatebox button{width:100%;margin-top:12px;padding:13px;border:0;border-radius:10px;background:var(--gold);color:var(--navy-d);font-weight:700;font-size:15px;cursor:pointer}
  .gatebox button:hover{background:var(--gold-l)}
  .gateerr{color:#ffd2cc;font-size:12px;height:16px;margin-top:10px}
  ...
  @media print{
    #gate,.nav,.printbtn,.sync{display:none !important}
...
function unlock(){
  if($("pw").value===PASSWORD){ try{ sessionStorage.setItem("dova_ok","1"); }catch(e){} start(); }
  else { $("gerr").textContent="Incorrect access code."; }
}

document.addEventListener("DOMContentLoaded", function(){
  var links=document.querySelectorAll(".nav a");
  for(var i=0;i<links.length;i++){ links[i].addEventListener("click", function(){ setPage(this.getAttribute("data-page"), this); }); }
  $("unlock").addEventListener("click", unlock);
  $("pw").addEventListener("keydown", function(e){ if(e.key==="Enter") unlock(); });
  var pre=false; try{ pre = sessionStorage.getItem("dova_ok")==="1"; }catch(e){}
  if(pre) start(); else $("pw").focus();
});
</script>
</body>
</html>`;
