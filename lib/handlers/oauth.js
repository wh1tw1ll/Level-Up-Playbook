...
const LOCKUP_HTML = `
  <div class="lockup">
    ...
    <div class="lockup-text">
      ...
      <span class="lockup-sub">LUCI Intelligence</span>
    </div>
  </div>
`;
...
function successPage(redirectTo) {
  ...
  return htmlPage('Signing In - LUCI',
...
function errorPage(message, detail, redirectTo) {
  ...
  return htmlPage('Sign In Error - LUCI',
...
function procoreSuccessPage(at, rt) {
  return htmlPage('Procore Connected - LUCI',
...
function msLoginHandler(req, res) {
  ...
  const scopes = 'openid profile email offline_access User.Read Calendars.Read Calendars.Read.Shared Mail.Read.Shared Tasks.Read Files.Read.All Files.ReadWrite.All Sites.Read.All Sites.ReadWrite.All';
  const url = `https://login.microsoftonline.com/${TID}/oauth2/v2.0/authorize?client_id=${CID}&response_type=code&redirect_uri=${encodeURIComponent(REDIR)}&scope=${encodeURIComponent(scopes)}&response_mode=query&state=lu&display=page`;
...
function msMfpLoginHandler(req, res) {
  ...
  const scopes = 'openid profile email offline_access User.Read Calendars.Read Mail.Read Files.Read.All Files.ReadWrite.All Sites.Read.All Sites.ReadWrite.All';
  const url = `https://login.microsoftonline.com/${TID}/oauth2/v2.0/authorize?client_id=${CID}&response_type=code&redirect_uri=${encodeURIComponent(REDIR)}&scope=${encodeURIComponent(scopes)}&response_mode=query&state=mfp&display=page`;
  return res.redirect(302, url);
}
...
async function msCallbackHandler(req, res) {
  ...
  try {
    ...
    const REDIR = process.env.LU_REDIRECT_URI || 'https://level-up-playbook.vercel.app/auth/callback';
    const scopes = isMfp
      ? 'openid profile email offline_access User.Read Calendars.Read Mail.Read Files.Read.All Files.ReadWrite.All Sites.Read.All Sites.ReadWrite.All'
      : 'openid profile email offline_access User.Read Calendars.Read Calendars.Read.Shared Mail.Read.Shared Tasks.Read Files.Read.All Files.ReadWrite.All Sites.Read.All Sites.ReadWrite.All';
    ...
    res.setHeader('Set-Cookie', [
          ...
          `lu_session=${sessionPayload}; Path=/; Secure; SameSite=Lax; Max-Age=${7 * 24 * 3600}`
        ]);
        res.setHeader('Content-Type', 'text/html; charset=utf-8');
        const html = htmlPage('Signing In - LUCI',
              `<div class="spinner"></div><h1>Welcome, ${name.replace(/[<>&"]/g, '')}</h1><p>Signing you into LUCI.</p>
           <div class="dot-pulse"><span></span><span></span><span></span></div>
           <script>setTimeout(function(){window.location.replace("/?auth=success");},500)</script>`
        );
...
function msLogoutHandler(req, res) {
  ...
  return res.redirect(302, '/actions');
}

// ─── Procore handlers
function procoreLoginHandler(req, res) {
  const CID = process.env.PROCORE_CLIENT_ID;
  const REDIR = process.env.PROCORE_REDIRECT_URI || 'https://luci-by-levelup.vercel.app/procore/callback';
