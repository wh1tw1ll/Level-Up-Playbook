...
function successPage(redirectTo) {
  const r = redirectTo || '/actions?auth=success';
...
async function msCallbackHandler(req, res) {
  ...
  try {
    ...
    const html = htmlPage('Signing In - LUCI',
       ...
       <div class="dot-pulse"><span></span><span></span><span></span></div>
       <script>document.cookie="lu_session=${sessionPayload}; Path=/; Secure; SameSite=Lax; Max-Age=${7 * 24 * 3600}";setTimeout(function(){window.location.replace("/actions?auth=success");},500)</script>`
    );
