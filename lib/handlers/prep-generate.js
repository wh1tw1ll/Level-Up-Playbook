...
async function ensureSheet() {
  ...
  if (reqTypeCol && reqTypeCol.options) {
    ...
    if (!opts.includes('AgendaGeneration')) {
      // Add it by updating the column — Smartsheet requires `type` when changing options
      await updateColumn(_sheetId, reqTypeCol.id, {
        type: reqTypeCol.type || 'PICKLIST',
        options: [...opts, 'AgendaGeneration'],
      });
