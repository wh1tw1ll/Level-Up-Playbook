...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Match agendas to events by subject
    const eventsWithAgendas = events.map(ev => {
      const match = agendas.find(a => {
        const es = (ev.subject || '').toLowerCase().trim();
        const as = (a.meetingSubject || '').toLowerCase().trim();
        return es === as || (es.length > 5 && as.length > 5 && (es.includes(as) || as.includes(es)));
      });
      return {
        ...ev,
        hasAgenda: !!match,
