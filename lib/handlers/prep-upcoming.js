...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Meetings that must NEVER receive a fuzzy-matched agenda (match too loose,
    // results in inaccurate agendas being attached). Keep the meeting, drop the agenda.
    const AGENDA_MATCH_DENYLIST = [
      'boldyn',
      'design team',
    ];
    ...
    function subjectMatches(evSubject, agSubject) {
      ...
      const es = String(evSubject).toLowerCase();
      // Denylist: these meetings never get a fuzzy-matched agenda
      if (AGENDA_MATCH_DENYLIST.some(token => es.includes(token))) return false;
