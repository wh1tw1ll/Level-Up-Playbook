...
export default async function handler(req, res) {
  ...
  try {
    // Vercel rewrites /api/dispatch/123456 to /api/index?path=dispatch/123456
    // So req.url is NOT the original URL. Parse rowId from query.path instead.
    const fullPath = req.query?.path || '';
    const parts = (Array.isArray(fullPath) ? fullPath[0] : fullPath).split('/').filter(Boolean);
    const rowId = parts[1]; // ['dispatch', '123456'] → '123456'
    if (!rowId) return res.status(400).json({ error: 'Missing rowId in URL path' });

    const source = req.query?.source || 'project';
    const type = req.query?.type || 'DO';
