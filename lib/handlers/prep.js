...
/** Fetch recent notes from Granola API with full detail (summaries, web_url) */
async function fetchGranolaNotes() {
  try {
    ...
    if (!res.ok) {
      console.error('Granola list fetch failed:', res.status);
      return [];
    }
    const data = await res.json();
    const cutoff = new Date(Date.now() - 14 * 86400000);
    const filtered = (data.notes || []).filter(n => new Date(n.created_at || 0) >= cutoff);
    console.log('Granola notes after date filter:', filtered.length);
    
    // Fetch individual details for up to 10 most recent notes (summaries, web_url)
    const enriched = await Promise.all(
      filtered.slice(0, 10).map(async (note) => {
        try {
          const detailRes = await fetch(`${GRANOLA_API}/notes/${note.id}`, {
            headers: {
              Authorization: `Bearer ${GRANOLA_TOKEN}`,
              'User-Agent': 'LUCI-App/1.0',
              'Accept': 'application/json',
          ...
          console.error('Granola detail fetch failed for', note.id, e.message);
        }
        return note;
      })
    );
    
    console.log('Granola enriched notes:', enriched.length);
    return enriched;
  } catch (err) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.json({
      meetings,
      agenda,
      perMeetingAgendas: await getStoredAgendas(),
    });
  } catch (err) {
